...
3. Branch on event:
   - **Successful deployment** (`deployment_status` with state `success` — Vercel and DigitalOcean report deploys this way): **evaluate.py** asks the LLM for *manual* test cases (Markdown) and posts them to Discord. No automated tests run.
   - **Failed deployment** (state `failure`/`error`): only a simple Discord notification (provider detected from the event payload); no diffing, tests, or tickets — exit 0.
   - **Push / PR**: **testgen.py** generates one pytest per changed file — Playwright for frontend files, `requests`-based API tests for backend (classified by extension/path heuristics). Each LLM response is code + a JSON summary separated by `===ARIA-SUMMARY===`; code is validated with `compile()` before writing to `testing/suites/generated/` alongside a `.json` summary.
